...
export const CardTimelineModal = ({ taskId, onClose }) => {
  ...
  const displayName = cardData?.nume_complet || cardData?.nume || cardData?.name || cardData?.denumire || cardData?.titlu || t.defaultCardTitle || 'FIȘĂ DOSAR CAMPANIE SARCINI';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-white shrink-0">
          <div>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t.lblCardInteractionHist || 'Istoric Interacțiuni Card'}</span>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
          {loading ? (
            <div className="text-center py-10 text-slate-500 font-bold animate-pulse">{t.lblLoadingHistory || 'Se încarcă istoricul...'}</div>
          ) : timeline.length === 0 ? (
            <div className="text-center py-10 text-slate-500">{t.msgNoHistorySaved || 'Nu există istoric salvat pentru acest card.'}</div>
          ) : (
                    ...
                    <div className="flex-1 bg-white border border-slate-200 p-4 rounded-xl shadow-sm">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-slate-500 font-medium">{t.lblCurrentStage || 'Etapă curentă:'} {item.step_name}</span>
                        </div>
                      ...
                      <div className="flex items-center gap-2 text-xs font-bold text-slate-600 bg-slate-50 px-3 py-2 rounded-lg mb-3">
                        ...
                        {item.profiles?.name || item.profiles?.email || t.lblUnknownUser || 'Necunoscut'}
                      </div>
                      ...
                      {item.notes ? (
                        ...
                        <div className="text-xs text-slate-400 italic">{t.msgNoNotesAdded || 'Nu a fost adăugată nicio notiță.'}</div>
                      )}
                    </div>
                  </div>
                );
