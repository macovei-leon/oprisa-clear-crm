...
export const RepetitiveHistoryModal = ({ flow, onClose }) => {
  ...
  const workerStats = history.reduce((acc, row) => {
    const workerName = row.profiles?.name || row.profiles?.email || (t.lblUnknown || 'Unknown');
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50 flex flex-col gap-6">
          ...
          <div className="flex items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <label className="font-bold text-slate-700">{t.lblChooseDate || 'Alege Data:'}</label>
            <input 
              type="date" 
              value={date} 
          ...
          {loading ? (
            <div className="text-center py-8 font-bold text-slate-500">{t.loadingData || 'Se încarcă datele...'}</div>
          ) : totalCompleted === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
              <CheckCircle2 size={48} className="mx-auto text-slate-300 mb-4" />
              <p className="font-bold text-slate-500 text-lg">{t.noActivityDate || 'Nu există activitate pentru data selectată.'}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-700 border-b border-slate-100 pb-2 mb-4">{t.totalTasksCompleted || 'Total Sarcini Finalizate:'} {totalCompleted}</h3>
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-700 border-b border-slate-100 pb-2 mb-4 flex items-center gap-2">
                  <Users size={18} className="text-slate-400" /> {t.operatorPerf || 'Performanță Operatori'}
                </h3>
                  ...
                  {Object.entries(workerStats).map(([worker, stats]) => (
                    <div key={worker} className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                      <div className="font-bold text-indigo-700 mb-2 flex justify-between">
                        <span>{worker}</span>
                        <span>{t.lblTotal || 'Total:'} {stats.total}</span>
                      </div>
