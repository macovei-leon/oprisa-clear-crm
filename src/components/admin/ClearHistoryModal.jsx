...
export const ClearHistoryModal = ({ flowId, onClose, onSuccess }) => {
  ...
  const handleClearHistory = async (e) => {
    ...
    try {
      ...
      if (userError || !user) throw new Error('Nu s-a putut obține utilizatorul curent.');
      ...
      if (signInError) {
        throw new Error('Parola este incorectă. Acces interzis.');
      }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200">
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-rose-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-slate-800">Curăță Istoricul</h2>
              <p className="text-xs font-bold text-slate-500 mt-0.5">Acțiune ireversibilă</p>
            </div>
          </div>
        ...
        <form onSubmit={handleClearHistory} className="p-6">
          <div className="mb-6">
            <p className="text-sm text-slate-600 mb-4">
              Ești pe cale să ștergi definitiv istoricul de operare pentru 
              <strong className="text-rose-600 ml-1">{flowId === 'all' ? 'TOATE fluxurile' : 'fluxul selectat'}</strong>.
              Pentru a confirma această acțiune cu un grad ridicat de risc, te rugăm să introduci parola contului tău de administrator.
            </p>
            ...
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Parola Administrator</label>
